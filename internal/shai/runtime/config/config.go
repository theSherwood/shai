...
func (c *Config) applyTemplates(env, vars map[string]string) error {
	var err error
	c.Image, err = expandTemplates(c.Image, env, vars)
	if err != nil {
		return fmt.Errorf("image: %w", err)
	}
	c.User, err = expandTemplates(c.User, env, vars)
	if err != nil {
		return fmt.Errorf("user: %w", err)
	}
	c.Workspace, err = expandTemplates(c.Workspace, env, vars)
	if err != nil {
	...
	for name, res := range c.Resources {
		for i := range res.Vars {
			res.Vars[i].Source, err = expandTemplates(res.Vars[i].Source, env, vars)
			if err != nil {
				return fmt.Errorf("resource %s var[%d] source: %w", name, i, err)
			}
			res.Vars[i].Target, err = expandTemplates(res.Vars[i].Target, env, vars)
			if err != nil {
				return fmt.Errorf("resource %s var[%d] target: %w", name, i, err)
			}
		}
		for i := range res.Mounts {
			res.Mounts[i].Source, err = expandTemplates(res.Mounts[i].Source, env, vars)
			if err != nil {
				return fmt.Errorf("resource %s mount[%d] source: %w", name, i, err)
			}
			res.Mounts[i].Target, err = expandTemplates(res.Mounts[i].Target, env, vars)
			if err != nil {
				return fmt.Errorf("resource %s mount[%d] target: %w", name, i, err)
			}
		}
		for i := range res.Calls {
			res.Calls[i].Description, err = expandTemplates(res.Calls[i].Description, env, vars)
			if err != nil {
				return fmt.Errorf("resource %s call[%d] description: %w", name, i, err)
			}
			res.Calls[i].Command, err = expandTemplates(res.Calls[i].Command, env, vars)
			if err != nil {
				return fmt.Errorf("resource %s call[%d] command: %w", name, i, err)
			}
		}
		for i := range res.HTTP {
			res.HTTP[i], err = expandTemplates(res.HTTP[i], env, vars)
			if err != nil {
				return fmt.Errorf("resource %s http[%d]: %w", name, i, err)
			}
		}
		for i := range res.Ports {
			res.Ports[i].Host, err = expandTemplates(res.Ports[i].Host, env, vars)
			if err != nil {
	...
	for i := range c.Apply {
		c.Apply[i].Path, err = expandTemplates(c.Apply[i].Path, env, vars)
		if err != nil {
			return fmt.Errorf("apply[%d] path: %w", i, err)
		}
		c.Apply[i].Image, err = expandTemplates(c.Apply[i].Image, env, vars)
		if err != nil {
...
func loadFromData(data []byte, path string, env, vars map[string]string) (*Config, error) {
	...
	if err := cfg.applyTemplates(env, vars); err != nil {
		return nil, err
	}
